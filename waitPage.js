module.exports = function renderWaitPage(transactionId, amount = 0) {
      ...
      <head>
        ...
        <style>
          * { box-sizing: border-box; margin: 0; padding: 0; }
          ...
          /* 🌟 النافذة المنبثقة عند إصدار الكارت */
      ...
      <body>
        ...
        <div class="modal-overlay" id="voucherModal">
          <div class="modal-box">
            ...
            <a href="${NETWORK_URL}" style="display: block; margin-top: 14px; color: #64748b; text-decoration: none; font-size: 13px; font-weight: 600;">التوجه للتصفح الآن <i class="fa fa-arrow-left"></i></a>
          </div>
        </div>
          ...
          async function checkVoucherStatus() {
            ...
            try {
              ...
              if (data.success && data.data) {
                ...
                // 🌟 التحقق إذا كان المبلغ أكبر من 100 جنيه لتحويله لصفحة المساهمة/الاهداء
                if (cardAmount > 100) {
                  window.location.href = '/contribution-success?amount=' + cardAmount + '&tx=' + encodeURIComponent(txId);
                  return;
                }

                // 🌟 إظهار نافذة الكارت المنبثقة للباقات العادية (100 جنيه أو أقل)
                document.getElementById('modalCardCode').innerText = data.data.code;
                document.getElementById('voucherModal').style.display = 'flex';
              } else {
                if (attempts < 80) setTimeout(checkVoucherStatus, 3000); // الفحص مستمر كل 3 ثوانٍ
              }
            } catch (e) {
