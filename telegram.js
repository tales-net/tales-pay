...
function getPaymentMethodName(data) {
    let method = data.paymentMethod || data.payment_method || data.source_type || data.method || 'محفظة إلكترونية';
...
/**
 * دالة إرسال الإشعارات النصية العامة إلى التليجرام
 */
export async function sendTelegramNotification(data) {
    try {
        ...
        // 1. حالة بدء محاولة الدفع (بيانات العميل والجهاز كاملة)
        if (data.type === 'PAYMENT_INITIATED' || (!data.type && data.clientID)) {
            ...
            message = `⏳ <b>جاري عملية الدفع - شبكة حكايات...</b>\n\n` +
        ...
        // 2. حالة نجاح الدفع وتأكيد العملية
        } else {
            const txnId = data.orderId || data.id || data.transactionId || data.order?.id || 'غير متوفر';
            const voucher = data.voucherCode || data.voucher_code || (data.card ? data.card.code : 'غير متوفر');
            const packageInfo = data.profile || data.package_info || data.packageName || 'باقة إنترنت شبكة حكايات';
...
/**
 * دالة إرسال صورة كارت الإنترنت والتنبيه بنفاد المخزون
 */
export async function sendVoucherWithCardImage(voucherInfo, imageBuffer) {
    try {
        if (!BOT_TOKEN || !CHAT_ID) {
            console.warn('⚠️ Telegram Bot Token or Chat ID is missing!');
            return;
        ...
        if (imageBuffer && card) {
            const captionText = `🎉 <b>تم دفع وتأكيد كارت الإنترنت بنجاح!</b>\n\n` +
                                `🌐 <b>شبكة حكايات نت</b>\n` +
                                `🆔 رقم المعاملة: <code>${transactionId || 'غير متوفر'}</code>\n` +
                                `📱 رقم الهاتف: <code>${phone || 'غير متوفر'}</code>\n` +
                                `📦 الباقة: <b>${packageName || 'باقة إنترنت'}</b> (${amount || 0} ج.م)\n` +
