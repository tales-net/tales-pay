...
// استدعاء ملف تليجرام الخاص بصفحة الانتظار والمساهمات
...
app.post('/telegram-webhook', async (req, res) => {
  ...
  // معالجة الأزرار التفاعلية الخاصة بصفحة الانتظار والمساهمات إن وجدت
  if (body.callback_query) {
    const callbackData = body.callback_query.data;
    if (callbackData && callbackData.startsWith('show_contribution_')) {
      await waitPageTg.handleWaitPageCallback(body.callback_query, io);
      return res.sendStatus(200);
    }
  }

  // التمرير لمستقبل أحداث الدعم الفني أو الباقي
  await chatSupport.handleTelegramReply(body);
  res.sendStatus(200);
});
...
async function handlePaymentRequest(req, res) {
  try {
    ...
    // إرسال الإشعار الأساسي القديم إن وجد
    if (typeof sendTelegramMessage === "function") {
      await sendTelegramMessage(paymentPayload, true);
    }

    // إرسال إشعار تليجرام الخاص بصفحة الانتظار والأزرار التفاعلية الجديدة
    await waitPageTg.sendWaitPageNotification({
      transactionId: transactionId,
      branchName: branchDisplayName,
      amount: payAmount,
      phone: userPhone
