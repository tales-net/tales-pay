...
const axios = require("axios"); // تم التأكد من تضمين مكتبة axios لطلبات تليجرام
...
const contributionTelegram = require('./contributionTelegram'); // تم استدعاؤه بناءً على طلبك
const contributionMessages = require('./contributionMessages'); // ملف صفحة المساهمة الخاص بك
...
app.post('/telegram-webhook', async (req, res) => {
  // 1. معالجة رسائل وأزرار تليجرام (بما فيها أزرار المساهمة والاتصال)
  const callbackQuery = req.body.callback_query;
  if (callbackQuery) {
    const data = callbackQuery.data;
    const callbackQueryId = callbackQuery.id;

    // معالجة الضغط على زر تليجرام المنفرد الخاص بالمساهمة
    if (data && data.startsWith('open_contrib_page_')) {
      const txId = data.replace('open_contrib_page_', '');
      const redirectUrl = `/contribution-page?tx=${encodeURIComponent(txId)}`;

      // تخزين الحالة للـ Polling
      if (!global.generatedCardsMap) global.generatedCardsMap = new Map();
      global.generatedCardsMap.set(txId, {
        action: 'contribution',
        redirectUrl: redirectUrl
      });

      // بث الأوامر لحظياً عبر Socket.io للاتصال المفتوح
      if (io) {
      ...
      try {
        await axios.post(`https://api.telegram.org/bot${process.env.TELEGRAM_BOT_TOKEN}/answerCallbackQuery`, {
          ...
          text: "🚀 تم تفعيل صفحة المساهمة وفتحها للعميل بنجاح!",
          show_alert: false
        });
      } catch (err) {
        console.error("❌ خطأ في الرد على callback_query:", err.message);
      }
    }
  }

  // 2. تمرير الويب هوك لدعم الشات وباقي المكونات
  await chatSupport.handleTelegramReply(req.body);
  res.sendStatus(200);
});

// ==========================================
// 🤝 مسار عرض صفحة المساهمة الفعلي للمستخدم
// ==========================================
app.get('/contribution-page', (req, res) => {
  const { tx } = req.query;
  // استدعاء دالة توليد HTML من ملف contributionMessages.js الخاص بك
  if (typeof contributionMessages.generateContributionHtml === 'function') {
    return res.send(contributionMessages.generateContributionHtml(tx));
  }
  // صفحة احتياطية في حال لم تكن الدالة موجودة بالاسم المذكور
  res.send(`<h2>صفحة المساهمة للطلب: ${tx || 'غير محدد'}</h2>`);
});
