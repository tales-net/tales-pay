...
async function handleTelegramReply(body) {
  try {
    ...
    if (clientId) {
      if (chatStatuses.get(clientId) === "closed") return;
      if (!chatSessions.has(clientId)) chatSessions.set(clientId, []);
      ...
      const adminMsgObj = { sender: "admin", text: replyText, image: adminImageUrl, timestamp: new Date() };
